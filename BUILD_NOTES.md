...
- **Fix**: Corrected `trip-events.json`: welcome-party → **Welcome Dinner at Hotel Condes de Barcelona** (Passeig de Gràcia 73-75, lat/lng 41.3936/2.162). Replaced `wedding-shuttles` + `wedding-ceremony` with 8 events: `wedding-hmu` (Angela H&MU 9:40), `wedding-groomsmen-photos` (Chandler 12:45), `wedding-minibus` (3:00 PM wedding-party buses from Grand Hotel Central), `wedding-guest-bus` (4:45 PM from Ronda Sant Pere 21-23, Segalés 59-seat, Mary +1 607-379-2239), `wedding-ceremony` (7:00–7:30 PM at El Convent de Blanes, lat/lng 41.6755/2.7997, city="Blanes"), `wedding-cocktail`, `wedding-dinner`, `wedding-party` (11:15 PM→2:15 AM, crosses midnight). Added `bus-barcelona-blanes` route (mode "bus" → solid line). Updated Trip_Planning_Document.md (overview, luggage row, both apartment location lists, Friday + Sunday timelines, Wedding Cheat Sheet), day-by-day.md, Barcelona.md, and the KML wedding pins. Regenerated ICS (34 events, 0 empty lines) + agent-context (269.6 KB).
- **⚠️ DO NOT REVERT**: The wedding is in **Blanes**, NOT the Gothic Quarter / Barcelona city. Do not "fix" the venue lat/lng back toward Barcelona, and do not re-add walking-distance claims to the venue from the apartments — it is a bus ride. **Chandler & Angela are in the wedding party** (in the hair/makeup list + processional per the PDF) → they take the **3:00 PM** mini buses from Grand Hotel Central; the **kids + Thomas family** take the **4:45 PM** guest bus. Keep these two transport paths distinct.
...
- `spain-2026.ics` wedding-day times are now **exact** (from Joe's final TIMING PDF, 2026-05-29) — no longer placeholders. The `wedding-party` event intentionally crosses midnight (May 31 23:15 → Jun 1 02:15) and both ends share `TZID=Europe/Madrid` (+02:00), which is valid; don't "split" it.
...
| 2026-05-26 | **Dedicated "Barajas → 14:30 AVE Chamartín" section + Combinado Cercanías free-ticket fact.** Verdict: **take Cercanías C1, not a taxi** — direct ~25 min AND free with the AVE ticket via Renfe's "Combinado Cercanías" QR scanner. Added a step-by-step subsection in `briefings/travel-playbook.md` (under Madrid Barajas → Departure block) with a 12-row time table from land-11:30 to depart-14:30, the Combinado machine walkthrough, taxi-fallback procedure, and critical reminders (e-gate kids rule, QR locks 2 min before, cancel fallback ticket once seated). Updated the Cercanías section to call out the Combinado FREE benefit. Added Barajas→Chamartín entry to the "I just landed" cheat sheet. Updated `trip-events.json` `gap-madrid-train-primary` recommendation and the equivalent Cercanías-options table in `Trip_Planning_Document.md` (around line 326) to mention Combinado and updated taxi fallback wording to "€66 (2 × €33 flat)". Regenerated `agent-context.json` (253.2 KB). | User: "I need to clean up the instructions and detailed airport to the 1430 train. Can we take a train? Or do we need to do Taxi?" Prior playbook briefly mentioned C1 but missed the headline Combinado free-ticket fact and lacked an end-to-end timeline. |
